...
#change all non-speed variables from fraction SVL yo %SVL (gap size is already in %SVL)
var_set <- grep("*_rel$", names(cdata), value = TRUE)
var_set <- var_set[c(-1,-6,-7,-8,-9,-10)]
for (x in var_set){
  cdata[,x] <- 100*cdata[,x]
}
...
#Decide random effects structure: slopes vs intercepts
zv.lmer <- lmer(zv_rel ~ gscm_relA + svl + (gscm_relA|ID), data = df, REML = TRUE) #model fails to converge
tt <- getME(zv.lmer,"theta")
ll <- getME(zv.lmer,"lower")
min(tt[ll==0])  #not close to 0
ss <- getME(zv.lmer,c("theta","fixef"))
zv.lmer1 <- update(zv.lmer,start=ss,control=lmerControl(optCtrl=list(maxfun=2e4))) #converges
zv.lmer2<- lmer(zv_rel ~ gscm_relA + svl + (1|ID), data = df, REML = TRUE)
anova(zv.lmer1,zv.lmer2)
#zv.lmer1 has lower AIC.
...
tt <- getME(av.lmer,"theta")
ll <- getME(av.lmer,"lower")
min(tt[ll==0])  #not close to 0
ss <- getME(av.lmer,c("theta","fixef"))
av.lmer1 <- update(av.lmer,start=ss,control=lmerControl(optCtrl=list(maxfun=2e4))) #converges
av.lmer2<- lmer(av_rel ~ gscm_relA + svl + (1|ID), data = df, REML = TRUE)
anova(av.lmer1,av.lmer2)
#av.lmer1 has lower AIC.
...
#Decide random effects structure: slopes vs intercepts
mv.lmer <- lmer(mv_rel ~ gscm_relA + svl + (gscm_relA|ID), data = df, REML = TRUE) #singular
mv.lmer1 <- lmer(mv_rel ~ gscm_relA + svl + (1|ID), data = df, REML = TRUE)

summary(mv.lmer1)
set.seed(500) #needed to ensure bootstraps are replicable 
confint(mv.lmer1, method = "boot", nsim = 1500) # The number of bootstrap iterations = 1500.
